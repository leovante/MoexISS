...
#import q_credentials.moex_db_secmaster_cred as db_cred                              # Подключение базы данных +
#import q_credentials.oanda_cred as oanda_cred
#import q_datafeeds.bt_datafeed_postgres as bt_datafeed_postgres_moex
import q_tools.args_parse_other as args_parse_other
from q_strategies import *
import q_strategies.simple_strategy
...
#
    ...
    dataname='/home/jovyan/work/q_pack/q_run/sber_test.csv',  # Файл для импорта
    ...
    cerebro.addstrategy(q_strategies.simple_strategy.St)
...
def parse_args(pargs=None):
    ...
    #парсит тикеры, по умолчанию ['EUR_USD,GBP_USD'] НЕ МЕНЯЕМ ПОКА
    parser.add_argument('--tickers', nargs='*' ,required=False,default=['EUR_USD,GBP_USD'], type=str,
                        help='Pass the tickers with space')
    #какие-то аргументы (можно добавить дополнительные ключи)НЕ МЕНЯЕМ ПОКА 
#    parser.add_argument('--dargs', default='',
#                        metavar='kwargs', help='kwargs in k1=v1,k2=v2 format')
#    # Начальная дата
#    parser.add_argument('--fromdate', required=False, default='2024-1-1',
#                        help='Date[time] in YYYY-MM-DD[THH:MM:SS] format')
#    # Конечная дата
#    parser.add_argument('--todate', required=False, default='2024-7-23',
#                        help='Date[time] in YYYY-MM-DD[THH:MM:SS] format')
#
